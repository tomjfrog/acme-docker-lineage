...
Local experiment against `https://tomjpd2.jfrog.io` validating golden-image lineage after rename **and** multi-hop derivation (`golden-base` → `payments-api` → `salestax-api`).
...
./lab/scripts/01-build-push.sh    # build/push golden, payments-api, salestax-api, renamed, non-golden
...
Per-beat scripts under `lab/scripts/steps/` remain for local runs and for workflows not yet rewritten as native YAML (01–03 are native). They do **not** replace `01-build-push.sh` / `02-detect-lineage.sh`.
...
| 03 | `03-publish-salestax-api-multihop.yml` | Multi-hop salestax-api; immediate-parent Evidence |
...
1. **Layer DiffID prefix** of golden on payments-api, salestax-api, and renamed → root is golden (Tier 2).
2. **Evidence walk** salestax-api → payments-api → golden-base → root is golden even when salestax-api’s predicate omits the golden digest (Tier 1 multi-hop).
